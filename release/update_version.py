...
def update_dockerfile(version, root_dir):
    """Update TAO_TOOLKIT_VERSION in both Dockerfile.release and Dockerfile.l4t.release."""
    dockerfile_files = [
        "release/docker/Dockerfile.release",
        "release/docker/Dockerfile.l4t.release"
    ]
    
    updated_files = []
    
    for dockerfile_rel_path in dockerfile_files:
        filepath = os.path.join(root_dir, dockerfile_rel_path)
        
        if not os.path.exists(filepath):
            print(f"ℹ Skipping {dockerfile_rel_path} (not found)")
            continue
        
        print(f"Updating {dockerfile_rel_path}...")
        ...
        with open(filepath, 'w') as f:
            f.write(content)
        print(f"✓ Updated {dockerfile_rel_path}")
        updated_files.append(dockerfile_rel_path)
...
def verify_updates(version, root_dir):
    ...
    # Check Dockerfile files (optional)
    dockerfile_files = [
        "release/docker/Dockerfile.release",
        "release/docker/Dockerfile.l4t.release"
    ]
    ...
    for dockerfile_rel_path in dockerfile_files:
        dockerfile_path = os.path.join(root_dir, dockerfile_rel_path)
        if os.path.exists(dockerfile_path):
            with open(dockerfile_path, 'r') as f:
                content = f.read()
            if f'ENV TAO_TOOLKIT_VERSION="{version}"' not in content:
                issues.append(f"{dockerfile_rel_path}: TAO_TOOLKIT_VERSION not updated correctly")
...
def main():
    ...
    try:
        ...
        dockerfile_updated = update_dockerfile(version, root_dir)
        
        # Verify updates
        success = verify_updates(version, root_dir)
        
        if success:
            files_updated = ["release/python/version.py"]
            if dockerfile_updated:
                files_updated.extend(["release/docker/Dockerfile.release", "release/docker/Dockerfile.l4t.release"])
