...
def get_requirements():
    ...
    with open(os.path.join(package_root, "docker/requirements-l4t.txt"), 'r') as req_file:
...
if "bdist_wheel" in sys.argv:
    if not any(arg.startswith('--plat-name') for arg in sys.argv):
        sys.argv.extend(['--plat-name', f"{__platform_system__.lower()}_{__platform_machine__}"])
...
setuptools.setup(
    ...
    entry_points={
        'console_scripts': [
            ...
            'changenet=nvidia_tao_deploy.cv.changenet.entrypoint.changenet:main',
