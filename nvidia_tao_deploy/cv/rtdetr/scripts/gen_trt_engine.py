...
@monitor_status(name='rtdetr', mode='gen_trt_engine')
def main(cfg: ExperimentConfig) -> None:
    ...
    # This will be useful at a time when we have RT-DETR with foundation model backbone.
    ...
    builder = DDETRDetEngineBuilder(**engine_builder_kwargs,
                                    workspace=workspace_size // 1024,  # RT-DETR config is set in MB. Converting to GB.
                                    )

    builder.create_network(tmp_onnx_file, file_format)
    builder.create_engine(**create_engine_kwargs)

    print("Engine build finished successfully.")
