...
from nvidia_tao_deploy.cv.rtdetr.utils import mscoco_label2category
...
@monitor_status(name='rtdetr', mode='evaluate')
def main(cfg: ExperimentConfig) -> None:
    """RT-DETR TRT evaluation.
    ...
    if cfg.dataset.remap_mscoco_category:
        logger.info("Predicted labels will be remapped to MSCOCO categories.")
        logger.info("WARNING: this is only applicable when your model is trained on MSCOCO dataset.")
    ...
    for imgs, scale, source_id, labels in tqdm(dl, total=len(dl), desc="Producing predictions"):
        ...
        if cfg.dataset.remap_mscoco_category:
            ...
            for i in range(b):
                for j in range(n):
                    class_labels[i, j] = mscoco_label2category[int(class_labels[i, j])]
        ...
        predictions['num_detections'].append(np.array([cfg.model.num_select] * cfg.dataset.batch_size).astype(np.int32))
    ...
    # Serialize metrics to a json file called results.json
