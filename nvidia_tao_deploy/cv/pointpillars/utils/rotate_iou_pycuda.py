...
"""CUDA implementation of IoU of rotated boxes."""
import numpy as np
import cupy as cp


mod = cp.RawKernel("""
    //CUDA implementation of IoU of rotated boxes. This is basically a C++/CUDA
    ...
    extern "C" __global__ void rotate_iou_gpu_eval(float *q, float *b, int q_max, int b_max, float *output, int criterion=-1)
    {
...
def rotate_iou_gpu_eval(box_np, query_np, criterion=-1):
    ...
    N = box_np.shape[0]
    K = query_np.shape[0]
    iou_np = cp.zeros((N, K), dtype=cp.float32)
    threadsPerBlock = 8 * 8
    blockspergrid = (div_up(N, threadsPerBlock), div_up(K, threadsPerBlock), 1)
    mod(blockspergrid, (threadsPerBlock, 1, 1), (cp.asarray(query_np), cp.asarray(box_np), np.int32(K), np.int32(N), iou_np, np.int32(criterion)))
    return cp.asnumpy(iou_np)
