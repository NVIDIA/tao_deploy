...
class OCRNetInferencer(TRTInferencer):
    ...
    def infer(self, imgs):
        ...
        # For OCRNet, there is just 1: [input]
        self._copy_input_to_host([imgs])

        # ...fetch model outputs...
        # 3 named results: [output_id, output_prob, 798]
        # (Note: 798 only appears in functional test and not when scripts are run)
        ...
        return trt_output_process_fn(results[:2])
