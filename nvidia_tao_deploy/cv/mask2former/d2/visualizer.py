...
class _PanopticPrediction:
    ...
    def __init__(self, panoptic_seg, segments_info, metadata=None):
        ...
        segment_ids, areas = np.unique(panoptic_seg, return_counts=True)
    ...
    def non_empty_mask(self):
        ...
        """
        empty_ids = []
        for idx in self._seg_ids:
            if idx not in self._sinfo:
...
class Visualizer:
    ...
    def draw_instance_predictions(self, predictions, mask_threshold=0.4):
        """
        ...
        if predictions.has("pred_masks"):
            masks = np.asarray(predictions.pred_masks)[keep]
    ...
    def draw_sem_seg(self, sem_seg, area_threshold=None, alpha=0.8):
        ...
        for label in filter(lambda lbl: lbl < len(self.metadata.stuff_classes), labels):
            try:
    ...
    def overlay_rotated_instances(self, boxes=None, labels=None, assigned_colors=None):
        ...
        sorted_idxs = np.argsort(-areas).tolist()  # pylint: disable=possibly-used-before-assignment
        # Re-order overlapped instances in descending order.
    ...
    def _change_color_brightness(self, color, brightness_factor):
        ...
        assert -1.0 <= brightness_factor <= 1.0, (
            f"Brightness factor {brightness_factor} not within [-1.0, 1.0]"
        )
        color = mplc.to_rgb(color)
    ...
    def _draw_text_in_mask(self, binary_mask, text, color):
        ...
        _num_cc, cc_labels, stats, _ = cv2.connectedComponentsWithStats(binary_mask, 8)
        if stats[1:, -1].size == 0:
            return
