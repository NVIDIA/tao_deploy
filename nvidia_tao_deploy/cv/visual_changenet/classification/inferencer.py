...
def trt_output_process_fn(y_encoded, diff_module='learnable'):
    """Function to process TRT model output."""
    predictions_batch = []
    for i in range(y_encoded.shape[0]):
        output = y_encoded[i]
        if diff_module == 'learnable':
            assert diff_module == 'learnable' and len(output) > 1, "Use the correct diff_module in model_config. Supported values are ['euclidean', 'learnable']"
...
class ChangeNetInferencer(TRTInferencer):
    ...
    def infer(self, input_images):
        ...
        pred = [i.reshape(max_batch_size, -1)[:actual_batch_size] for i in results]
        pred = trt_output_process_fn(pred[0], self.diff_module)
        return pred
