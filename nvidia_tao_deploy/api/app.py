...
from nvidia_tao_deploy.api.api_utils import microservice_utils, module_utils, ngc_utils, process_queue, json_schema_validation, dataclass2json_converter
from nvidia_tao_deploy.api.api_utils.common_utils import remove_none_empty_fields
...
def get_actions(neural_network_name):
    ...
    get:
      ...
      - name: neural_network_name
        ...
        schema:
          type: string
          enum: [ "centerpose", "classification_pyt", "classification_tf1", "classification_tf2", "deformable_detr", "detectnet_v2", "dino", "efficientdet_tf1", "efficientdet_tf2", "faster_rcnn", "grounding_dino", "lprnet", "mask_grounding_dino", "mask_rcnn", "mask2former", "metric_learning_recognition", "multitask_classification", "ocdnet", "ocrnet", "optical_inspection", "retinanet", "segformer", "ssd", "unet", "visual_changenet", "yolo_v3", "yolo_v4" ]
      responses:
        200:
...
def get_schema(neural_network_name, action_name):
    ...
    get:
      ...
      - name: action_name
        ...
        schema:
          type: string
          enum: [ "gen_trt_engine", "evaluate", "inference"]
      responses:
        200:
    ...
    try:
        ...
        imported_module = dataclass2json_converter.import_module_from_path(f"{'.'.join(ep_module[:3])}.hydra_config.default_config")
        expConfig = imported_module.ExperimentConfig()
        json_with_meta_config = dataclass2json_converter.dataclass_to_json(expConfig)
        json_schema = dataclass2json_converter.create_json_schema(json_with_meta_config)
...
def post_action(neural_network_name, action_name):
    ...
    # Removing empty and None fields
